...
const Shipment = ({user,cart, addToCart}) => {
            ...
            <Row className="mt-5 align-items-center mx-auto">
                <Col style={{display: shippingData ? 'none' : 'block'}}>
                    <form className="form-group" onSubmit={handleSubmit(onSubmit)} placeholder="Enter name">
                        <input style={{ width:'300px'}} className="form-control" name="name" defaultValue={user.displayName || ''} ref={register({ required: true })} placeholder="Enter Name"/>
                        {errors.name && <span>Name is required</span>}

                        <br/>
                        <input style={{ width:'300px'}} size="md" className="form-control" name="email" defaultValue={user.email || ''} ref={register({ required: true })}  placeholder="Enter email"/>
